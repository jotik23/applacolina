...
class PurchaseRecord:
    ...
    support_group_code: str | None
    support_group_leader_id: int | None
    support_group_leader_code: str | None
    is_support_group_leader: bool
...
def _build_pagination(
    *,
    ...
    group_summaries = _build_group_summaries(page.object_list)
    records = tuple(
...
def _build_group_summaries(
    purchases: Sequence[PurchaseRequest],
) -> dict[str, dict[str, Decimal | str | None]]:
    group_codes = {p.support_group_code for p in purchases if p.support_group_code}
    if not group_codes:
        return {}
    members = list(
        PurchaseRequest.objects.filter(support_group_code__in=group_codes).select_related('expense_type')
    )
    summaries: dict[str, dict[str, Decimal | str | None]] = {}
    for member in members:
        code = member.support_group_code
        if not code:
            continue
        summary = summaries.setdefault(
            code,
            {
                'currency': member.currency or 'COP',
        ...
        estimated = member.estimated_total or Decimal('0.00')
        invoice_total = member.invoice_total or Decimal('0.00')
        payment_amount = member.payment_amount or Decimal('0.00')
        summary['estimated_total'] = summary['estimated_total'] + estimated
        summary['invoice_total'] = summary['invoice_total'] + invoice_total
        summary['payment_amount'] = summary['payment_amount'] + payment_amount
        ranking_amount = invoice_total if invoice_total > 0 else estimated
        if ranking_amount > summary['dominant_amount']:
            summary['dominant_amount'] = ranking_amount
            summary['category_name'] = member.expense_type.name if member.expense_type else summary['category_name']
    for summary in summaries.values():
        summary.pop('dominant_amount', None)
    return summaries
...
def _build_purchase_record(
    ...
    *,
    ...
    group_summaries: dict[str, dict[str, Decimal | str | None]] | None = None,
) -> PurchaseRecord:
    requester_name = ""
    ...
    summary = None
    if group_summaries and purchase.support_group_code:
        summary = group_summaries.get(purchase.support_group_code)
    base_total = purchase.invoice_total if purchase.invoice_total and purchase.invoice_total > 0 else purchase.estimated_total
    total_amount = base_total or Decimal('0')
    paid_amount = purchase.payment_amount or Decimal('0')
    show_payment_breakdown = purchase.show_payment_breakdown
    support_group_code = purchase.support_group_code or None
    leader_obj = purchase.support_group_leader
    is_support_group_leader = bool(support_group_code and leader_obj is None)
    support_group_leader_id = None
    support_group_leader_code = None
    if support_group_code:
        if is_support_group_leader:
            support_group_leader_id = purchase.pk
            support_group_leader_code = purchase.timeline_code
        elif leader_obj:
            support_group_leader_id = leader_obj.pk
            support_group_leader_code = leader_obj.timeline_code
    category_name = purchase.expense_type.name
    if summary:
        summary_invoice = summary['invoice_total']  # type: ignore[index]
        summary_estimated = summary['estimated_total']  # type: ignore[index]
        total_amount = summary_invoice if summary_invoice > 0 else summary_estimated
        paid_amount = summary['payment_amount']  # type: ignore[index]
        category_name = summary.get('category_name') or category_name
    return PurchaseRecord(
        ...
        support_group_leader_id=support_group_leader_id,
        support_group_leader_code=support_group_leader_code,
        is_support_group_leader=is_support_group_leader,
    )
