...
Aunque el modelo `UserProfile` vive en la app `personal`, el calendario depende de algunos atributos:
...
El archivo `personal/fixtures/initial_calendario.json` ofrece un dataset mínimo (granjas, operarios, posiciones y un calendario de muestra) para explorar la interfaz. Todas las cuentas usan la contraseña `calendario123` y deben emplearse únicamente en entornos de desarrollo.

```bash
python manage.py loaddata personal/fixtures/initial_calendario.json
```
